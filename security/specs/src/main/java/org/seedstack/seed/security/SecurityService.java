...
public interface SecurityService {
     ...
     */
    <T> Collection<Principal<T>> getPrincipals();
     ...
     * @return true if user has the given permission, false otherwise.
     */
    boolean hasPermission(String permission, Scope... scopes);
     ...
     *
     * @param role the id of the role to test. Not null.
     ...
     * @return true if the user has the role, false otherwise.
     */
    boolean hasRole(String role, Scope... scopes);

    /**
     * Gives the roles given to the user. You can find all the scopes of a Role by calling role.getScopes() or the
     * scopes of a specific type (like SimpleScope) by calling role.getScopesByType(SimpleScope.class)
     *
     * @return a Set of all the roles the user has (empty if none).
     */
    Set<Role> getRoles();
     ...
     * invalidation. Because cookies are part of the HTTP header, and headers can only be set before the response
     * body (html, image, etc) is sent, this
     * method in web environments must be called before <em>any</em> content has been rendered.
     * <p>
     * The typical approach most applications use in this scenario is to redirect the user to a different location (e
     * .g. home page) immediately after calling this method. This is an effect of the HTTP protocol itself and not a
     ...
     * @see SecurityService#isRemembered()
     */
